...
import { MessageCenterWorkspace } from "./message-center-workspace";
...
export type StaffPortalPage = "dashboard" | "students" | "student-detail" | "student-new" | "customer-devices" | "new-order" | "enrollment-new" | "message-center" | "brochures" | "order-detail" | "application-detail" | "settings" | "profile";
...
function pageNavigationLabel(page: StaffPortalPage) {
  ...
  if (page === "message-center") return "Message Center";
      ...
      <aside className={`sidebar ${sidebarOpen ? "open" : ""}`}>
        ...
        <nav className="sidebar-nav" aria-label="Staff navigation">
          {visibleNavigation.map((group) => <div key={group.label}><p className="nav-label">{group.label}</p><ul className="nav-list">{group.items.map((item) => {
            ...
            const href = item.label === "Dashboard" ? "/staff" : item.label === "Students" ? "/staff/students" : item.label === "Customer Devices" ? "/staff/customer-devices" : item.label === "New Orders" ? "/staff/new_order" : item.label === "Message Center" ? "/staff/message-center" : item.label === "Brochures" ? "/staff/brochures/new" : item.label === "Settings" ? "/staff/settings" : null;
